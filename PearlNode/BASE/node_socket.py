...
class PearlNodeSocket(bpy.types.NodeSocket, SocketBase):
    bl_idname = 'PearlNodeSocket'
    bl_label = 'PearlNodeSocket'

    socket_color = (0.5, 0.5, 0.5, 1)

    compatible_sockets = ['PearlNodeSocketResult']

    text: bpy.props.StringProperty(default='')
    default_value: bpy.props.IntProperty(default=0, update=update_node)
...
class PearlNodeSocketFloat(PearlNodeSocket, SocketBase):
    bl_idname = 'PearlNodeSocketFloat'
    bl_label = 'PearlNodeSocketFloat'

    socket_color = (0.5, 0.5, 0.5, 1)

    default_value: bpy.props.FloatProperty(default=0, update=update_node)


class PearlNodeSocketString(PearlNodeSocket, SocketBase):
    bl_idname = 'PearlNodeSocketString'
    bl_label = 'PearlNodeSocketString'

    socket_color = (0.2, 0.7, 1.0, 1)

    default_value: bpy.props.StringProperty(default='', update=update_node)



class PearlNodeSocketResult(PearlNodeSocket, SocketBase):
    bl_idname = 'PearlNodeSocketResult'
    bl_label = 'PearlNodeSocketResult'

    socket_color = (1, 1, 1, 0.5)

    compatible_sockets = [PearlNodeSocket.bl_idname,
                          PearlNodeSocketFloat.bl_idname,
                          PearlNodeSocketString.bl_idname,
    ]


classes = [
    PearlNodeSocket,
    PearlNodeSocketFloat,
    PearlNodeSocketString,


    PearlNodeSocketResult,
]
